...
const KIND_STYLE: Record<string, string> = {
  ...
  chem_update: "bg-neutral-700 text-neutral-200",
      ...
      {state.ledger_entries.length === 0 ? (
        ...
        <ol className="space-y-3">
          {state.ledger_entries.map((e) => (
            <li key={e.id} className="rounded border border-neutral-800 bg-neutral-900 p-4">
              <div className="flex items-center justify-between">
                <span className={`rounded px-2 py-0.5 text-xs ${KIND_STYLE[e.kind] ?? "bg-neutral-700"}`}>
                  {e.kind.replace(/_/g, " ")}
                </span>
              ...
              {e.content && (
                <pre className="mt-1 whitespace-pre-wrap font-sans text-xs text-neutral-400">
                  {e.content}
                </pre>
              )}
            </li>
