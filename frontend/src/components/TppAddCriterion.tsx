...
export function TppAddCriterion({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="w-full max-w-lg rounded-lg border border-neutral-700 bg-neutral-950 p-6"
        ...
        <label className="mb-1 block text-xs text-neutral-400">Property</label>
        <select value={metric} onChange={(e) => pick(e.target.value)}
          className="mb-3 w-full rounded border border-neutral-700 bg-neutral-900 px-2 py-1.5 text-sm">
        ...
        <div className="mb-3 flex items-center gap-2 text-sm">
          <span className="text-neutral-400">Criterion:</span>
          ...
          <input value={threshold} onChange={(e) => setThreshold(e.target.value)} placeholder="threshold"
            className="w-28 rounded border border-neutral-700 bg-neutral-900 px-2 py-1 font-mono" />
          <span className="text-neutral-500">{def?.units}</span>
        </div>

        <label className="mb-1 block text-xs text-neutral-400">Rationale (optional)</label>
        <textarea value={rationale} onChange={(e) => setRationale(e.target.value)} rows={2}
          placeholder="Why this criterion matters…"
          className="mb-3 w-full rounded border border-neutral-700 bg-neutral-900 p-2 text-sm" />
        ...
        <label className="mb-1 block text-xs text-neutral-400">Justification (required)</label>
