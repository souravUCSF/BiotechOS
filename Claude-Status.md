...
- **Anonymizer scope narrowed (commit 9aa89ca):** now anonymizes ONLY target (TGTA/TGTA→TGTA, TGTA→TGTB, TGTA→Kinase-C), SMILES/structures, chemical images (dropped), and residue/mutation/position callouts (V600E, Y340D, Cys805) — across body/subject/filenames/slugs. Vendor names, people, domains, phones, codes are KEPT REAL. Therefore **`data/corpus/` is gitignored** (real third-party PII) and regenerated locally via `ingest.anonymize.build_corpus`; the anonymize-corpus skill + extract vendor map updated to real domains. Leak-scan = 0 for targets/mutations in paths + contents.
- **Evals (commit 9aa89ca):** `backend/biotechos/evals/` — LLM-judge-primary + deterministic guardrails (groundedness, include/exclude); suites qa/classify/fields/identity/decision over `data/evals/*.jsonl`; runner + baseline regression diff + `capture`/`sample` authoring helpers. Seed cases + baseline (15/15) + README committed. Run: `uv run python -m biotechos.evals run`. `data/evals/results/` gitignored.
- **Phase 2 (Current Inbox) — DONE (commit bbfd6c5):** ingest stages inbox items; approve() branches Quote→PO, Data→DB (merges surrogate/real-coded molecules into BTX set), Query→grounded reply; `GET /inbox` with context panel; rebuilt Current Inbox page. QueryOS also got clickable source panels.
- **Next:** rebuild `/` as email-envelope items (triaged; noise hidden) with extraction + analysis + context panel + decision branches (Quote→PO, Data→DB via `resolve_molecule(create=True)` merging surrogate-coded molecules into the BTX set, Query→reply); approval commits observations→facts + Decision Log. **Phase 3:** Tasks tab + invoice/contract/logistics agents.
